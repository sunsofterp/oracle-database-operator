...
4. Add the following fields to the Autonomous Database resource definition. An example `.yaml` file is available here: [`config/samples/adb/autonomousdatabase_create.yaml`](./../../config/samples/adb/autonomousdatabase_create.yaml)
    ...
    | `spec.details.databaseEdition` | string | The Oracle Database edition that applies to a BYOL Autonomous Database Serverless. The following values are valid:<br> `STANDARD_EDITION` - Oracle Database Standard Edition 2<br> `ENTERPRISE_EDITION` - Oracle Database Enterprise Edition<br> Only honoured when `licenseModel` is `BRING_YOUR_OWN_LICENSE`; if not specified, OCI defaults to `ENTERPRISE_EDITION`. | No |
...
1. Add the following fields to the AutonomousDatabase resource definition. An example YAML file is available here: [config/samples/adb/autonomousdatabase_clone.yaml](./../../config/samples/adb/autonomousdatabase_clone.yaml)
    ...
    | `spec.clone.databaseEdition` | string | The Oracle Database edition of the clone for a BYOL Autonomous Database Serverless: `STANDARD_EDITION` (Standard Edition 2) or `ENTERPRISE_EDITION`. Only honoured when `licenseModel` is `BRING_YOUR_OWN_LICENSE`. | No |
